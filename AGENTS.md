...
- **Do not attach a Turbopack loader rule broadly over `*.{ts,tsx,js,...}`.** Worker-backed modules such as the contact canvas generate virtual modules that broad rules may incorrectly treat as project files. Keep loader rules narrowly scoped to their real asset extensions.
...
- **Request-time logic goes in `src/proxy.ts`, not in pages.** Reading `headers()`/`cookies()` at the top of a page forces it dynamic under Cache Components. Keep pages prerenderable; do per-request work (UA redirects, content negotiation) in the matcher-scoped proxy.
