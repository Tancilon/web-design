...
const Logo = memo(({ className }: { className?: string }) => {
  ...
  const handleCopy = useCallback(() => {
    try {
      navigator.clipboard.writeText("FeiFei的个人作品集")
    } catch (error) {
      console.error("Error copying portfolio title:", error)
    }
  }, [])
  ...
  return (
    <>
      <div className="flex items-center" onContextMenu={handleContextMenu}>
        <span
          className={cn(
            "inline-block whitespace-nowrap font-display text-[0.875rem] font-semibold leading-none lg:-translate-y-0.5 lg:text-base",
            className
          )}
        >
          FeiFei的个人作品集
...
export const NavbarContent = memo(({ links }: NavbarContentProps) => {
    ...
    >
      <div className="grid-layout h-9">
        <button
          ...
          className="col-span-1 flex h-full w-fit items-center lg:col-start-1 lg:col-end-3"
          aria-label="返回首页"
        >
          <Logo className="text-brand-w1" />
        </button>

        <DesktopContent links={links} />

        <MobileContent links={links} />
      </div>
    </nav>
  )
})
...
const DesktopContent = memo(({ links }: NavbarContentProps) => {
  ...
  return (
    <>
      <div className="col-start-3 col-end-11 hidden w-full justify-center gap-5 font-display lg:flex">
        {links.map((link) => (
          <div
            key={link.href}
            className={cn(
              "flex items-center gap-1 text-[0.875rem] font-semibold leading-5"
            )}
          >
            <Link
              href={link.href}
      ...
      <div className="col-start-11 col-end-13 ml-auto hidden items-center gap-5 font-display lg:flex">
        <button
          id="nav-contact"
          onClick={handleContactButton}
          className={cn(
            "text-[0.875rem] font-semibold leading-5 text-brand-w1 transition-colors duration-0 hover:text-brand-o",
            isContactOpen && "text-brand-g1"
          )}
        >
          <span className="actionable-opacity">
            {SITE_ROUTE_LABELS["/contact"]}
          </span>
        </button>
      </div>
    </>
...
const MobileContent = memo(({ links }: NavbarContentProps) => {
  ...
  }, [isOpen, focusTrapRef, mobileMenuRef, isDesktop, links])
